...
| [`rtorrent/`](./rtorrent/) | rTorrent, el cliente BitTorrent de ncurses, con [`rtorrent.rc`](./rtorrent/rtorrent.rc) propia (verificada línea por línea contra el binario 0.16.19) y una carpeta dedicada en `~/Downloads/rtorrent/`: `downloads/` para los datos y `watch/`, donde dejar un `.torrent` lo arranca solo. La config se **copia**, no se enlaza; `status` avisa si la desplegada quedó distinta de la del repo. `uninstall` borra el paquete y la config, pero **nunca** las descargas. | `./rtorrent/rtorrent.sh install \| uninstall \| status` (sin argumentos: menú) |
...
| [`voxtype/`](./voxtype/) | Dictado por voz push-to-talk (voxtype) + backend de tipeo `wtype`. Sirve tanto a Arch (AUR `voxtype-bin`) como a Fedora (`.rpm` oficial de GitHub Releases, verificada por SHA256), y deja el daemon como servicio de usuario habilitado, el modelo Whisper `base.en` bajado y la config corregida (`model = "base.en"`, hotkey interno desactivado porque la activación va por bind del compositor). `status` reporta paquete, daemon, modelo, config y backend GPU/CPU activo. | `./voxtype/voxtype.sh install \| uninstall \| status` (sin argumentos: menú) |
