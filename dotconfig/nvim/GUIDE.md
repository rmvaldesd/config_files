...
`go` and `delve` (the `dlv` debugger) come from pacman, and `gopls` from
`go install`, both handled by
[`archdesktopinstall.sh`](../../archdesktopinstall.sh) — sections 5 and 14.
Nothing to do here if you ran it.

> Don't `go install` delve on top of the packaged one. `~/go/bin` comes *before*
> `/usr/bin` on the `PATH`, so a hand-installed `dlv` silently shadows it and
> stops getting system updates.
