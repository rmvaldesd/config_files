...
**must be on your `PATH`** — added in `~/.zshrc.local`, not `~/.zshrc`, which is
generated by oh-my-zsh and no longer versioned.

### Go

Everything is handled by
[`archdesktopinstall.sh`](../../archdesktopinstall.sh) — nothing to install by
hand. From pacman (section 5): `go`, `delve` (the `dlv` debugger), `go-tools`
(gives `goimports`), `gofumpt` and `staticcheck`. Via `go install` (section 14):
`gopls`, which is the only one not packaged in the official repos.

> Don't `go install` these on top of the packaged ones. `~/go/bin` comes
> *before* `/usr/bin` on the `PATH`, so a hand-installed `dlv` or `gofumpt`
> silently shadows the packaged one and stops getting system updates.

`goimports` and `gofumpt` are the Go formatters declared in
[`format.lua`](lua/plugins/format.lua); conform calls the **binaries**, so they
have to exist even though gopls embeds its own gofumpt. `staticcheck` changes
nothing inside nvim — gopls already runs its embedded analyzers via
`staticcheck = true` — it's there for running the linter from the terminal.
...
- `debugpy` **must** live in the same virtualenv that runs your app, or
  breakpoints never fire. The system `python-debugpy` from the installer only
  covers the fallback path in [`dap.lua`](lua/plugins/dap.lua), when there's no
  venv and it lands on `python3`. Same story for `pytest`.
- `black` and `isort` (the formatters in `format.lua`) come from pacman. Without
  them saving a `.py` formats *nothing*: pyright has no formatter, so conform's
  `lsp_format = "fallback"` has nothing to fall back to.

### direnv

`direnv` is installed and hooked in `~/.zshrc.local`. Drop a one-line `.envrc`
in a project to get its venv activated on `cd`:

```bash
echo 'layout python' > .envrc
direnv allow                       # required once; an .envrc is executable code
```
