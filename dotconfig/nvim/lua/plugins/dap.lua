return {
	...
	config = function()
		...
		require("dap").configurations.go = {
			{
				...
				substitutePath = {
					{
						from = "${env:WORKSPACE_ROOT}/src/code.uber.internal/devexp/green-keeper/controller/dependency/repo_insights.go",
						to = "bazel-out/k8-dbg/bin/src/code.uber.internal/devexp/green-keeper/controller/dependency/cff_/repo_insights_cffgen.go",
					},
					{
		...
		vim.keymap.set("n", "<leader>lb", function()
			vim.ui.input({ prompt = "Mensaje para log breakpoint: " }, function(input)
				if input then
